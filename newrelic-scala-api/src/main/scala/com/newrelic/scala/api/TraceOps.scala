...
object TraceOps {
   ...
   *    o1.map(traceFun("trace map segment")(i => i + 1))
   * }
   * </pre>
   * e.g. the code below will create a Transaction and with 2 segments <i>trace future creation</i> and <i>trace filter future</i>
   * <pre>
   * txn {
   *    Future(1)
   *        .map(traceFun("trace map segment")(i => i + 1))
